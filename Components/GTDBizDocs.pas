...
//    GTD_STD_DATETIMEMASK        = 'DD-MMM-YYY HH:NN:SS';
	...
	// -- Information area
	GTD_PL_INFORMATION_NODE     = '/PriceList/Information';
	...
	GTD_PL_PRODUCTITEM_NODE     = '/Product Item';
	GTD_PL_PRODUCTITEMS_TAG     = 'Product Items';
	GTD_PL_PRODUCTITEM_TAG      = 'Product Item';
	GTD_PL_ELE_PRODUCT_CODE     = 'Product_ID';     // <== Change to PLU (PRoduct Lookup)
	...
	GTD_PL_ELE_PRODUCT_SELL     = 'Your_Price';     // <= This is the depraceted definition
	GTD_PL_ELE_PRODUCT_ACTUAL   = 'Your_Price';     // <= and here is the new
	...
	GTD_CURRENT_PRICELIST       = 'Retail' + GTD_PRICELIST_EXT;
...
begin
    ...
    begin
        ...
        if t_Ref = '' then
        begin
            problist := problist + 'Missing Trader GTL (' + GTD_PL_VENDORINFO_NODE + '/' + GTD_PL_ELE_COMPANY_CODE + ')' + #13;
        end
        else if t_Name = '' then
        begin
...
begin
    ...
    CountryList.Add('CÙte D''ivoire (Ivory Coast)' );
...
begin
    ...
    if CountryName = 'CÙte D''ivoire (Ivory Coast)' then Result := 'CI' else
    ...
    if CountryName = 'RÈunion'                      then Result :=  'RE' else
...
begin
    ...
    if CountryCode = 'CI' then Result := 'CÙte D''ivoire (Ivory Coast)' else
    ...
    if CountryCode = 'RE' then Result := 'RÈunion' else
...
begin
	...
	// CSIDL_BITBUCKET Ç≤Ç›î
	// CSIDL_DESKTOPDIRECTORY ÉfÉXÉNÉgÉbÉv
	// CSIDL_FAVORITES Ç®ãCÇ…ì¸ÇË
	// CSIDL_PERSONAL É}ÉCÉhÉLÉÖÉÅÉìÉg
	// CSIDL_PROGRAMS ÉXÉ^Å[ÉgÉÅÉjÉÖÅ[ÇÃÉvÉçÉOÉâÉÄÉtÉHÉãÉ_
	// CSIDL_RECENT ç≈ãﬂégÇ¡ÇΩÉtÉ@ÉCÉã
	// CSIDL_SENDTO ëóÇÈ
	// CSIDL_STARTMENU ÉXÉ^Å[ÉgÉÅÉjÉÖÅ[
	// CCSIDL_STARTUP
