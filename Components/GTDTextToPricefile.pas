...
begin
    ...
    begin
        ...
        if (c <> '') then
            r := r + EncodeStringField(GTD_PL_ELE_PRODUCT_CODE,c) + #13 + #10;
        ...
        if (sp <> '') then
            r := r + '          ' + EncodeStringField(GTD_PL_ELE_PRODUCT_SELL,sp);
...
begin
    ...
    begin
        ...
        begin
            ...
            if (foundtabs = FieldNumber) then
                // -- We have found the start of our field
                fieldstartpos := xc + 1
            else if (foundtabs = FieldNumber+1) then
            begin
...
begin
    ...
    if (cn <> -1) then
    begin
        if (LogicalplField = GTD_PL_ELE_PRODUCT_CODE) then
        begin
            fCodePos := cn;
        end
        ...
        else if (LogicalplField = GTD_PL_ELE_PRODUCT_SELL) then
        begin
            fBuyPos := cn;
            //fBuyLen,
        end
...
procedure Register;
begin
  RegisterComponents('Tradalogs', [GTDPricefileConvertor]);
end;
